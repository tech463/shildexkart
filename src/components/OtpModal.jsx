...
export default function OtpModal({ open, email, password, onClose, onVerified }) {
  ...
  const verifyOtp = (event) => {
    ...
    ;(async () => {
      try {
        ...
        // token response me aata hai (backend SS)
  ...
  const resendOtp = () => {
    if (seconds > 0) return
    if (!password?.trim()) {
      // Resend ke liye login step same payload use karta hai
      setError('Resend OTP requires password. Please go back and enter password again.')
      return
    }
    ...
    ;(async () => {
      try {
        const data = await loginAPI({ email, password: password.trim() })
        if (!data?.success) {
